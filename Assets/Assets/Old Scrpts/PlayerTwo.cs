...
public class PlayerTwo : MonoBehaviour
{
    ...
    [SerializeField] private float maxLife = 550;
    ...
    void Update()
    {
        if (GameManager.Instance.p1 == 0 && GameManager.Instance.pauseState == false)
        {
            MovementControl();
            JumpControl();
            AttackControl();
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            isJumping = false;
            anim.SetBool("isJumping2", isJumping);
            anim.SetBool("TakingDamage2", false);
        }
    }
    public void LifeBar()
    {
        lifeBar2.fillAmount = life / maxLife;
        //if (life == 0)
    }
    public void Shoot()
    {
    ...
    {
        if (Input.GetKeyDown(KeyCode.Keypad0) && !isJumping)
        {
    ...
    public void TakingDamage(float damage)
    {
        ...
        anim.SetBool("TakingDamage2", true);
        life -= damage;
        if (life <= 0)
        {
            GameManager.Instance.pause = 0;
            GameManager.Instance.p2++;
